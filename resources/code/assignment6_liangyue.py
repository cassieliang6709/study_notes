...
def get_sentence_vectorizer(sentences):
    ...
    # standardize=None preserves original casing and punctuation — important for NER
    # (capitalized words are strong signals for named entities)
...
def label_vectorizer(labels, tag_map):
    ...
    # Pad the elements (post-padding with 0 so padding positions are the reserved mask index)
    label_ids = tf.keras.utils.pad_sequences(label_ids, padding='post')
...
def NER(len_tags, vocab_size, embedding_dim = 50):
    ...
    model.add(
        tf.keras.layers.Embedding(
            ...
            mask_zero=True          # masks padding (index 0) from LSTM
        )
    )
    ...
    model.add(
        tf.keras.layers.LSTM(
            units=embedding_dim,
            return_sequences=True   # output a label for every token, not just the last
        )
    )
    ...
    model.add(
        tf.keras.layers.Dense(
            units=len_tags,
            activation=tf.nn.log_softmax  # pass the function object, not tf.nn.log_softmax()
        )
    )
...
def masked_loss(y_true, y_pred):
    ...
    # The Dense layer outputs log-probabilities (log_softmax).
    # SparseCategoricalCrossentropy with from_logits=False expects probabilities in [0,1],
    # so we exponentiate to convert log-probs → probs before passing to the loss.
    loss_fn = tf.keras.losses.SparseCategoricalCrossentropy(
        from_logits=False,
        reduction='none'
    )
    loss = loss_fn(y_true, tf.exp(y_pred))  # shape: (batch, seq_len)

    # Build mask: ignore positions where y_true == 0 (padding token)
    mask = tf.cast(tf.not_equal(y_true, 0), dtype=tf.float32)

    # Apply mask and compute mean over non-padding tokens
    loss = tf.reduce_sum(loss * mask) / tf.maximum(tf.reduce_sum(mask), 1.0)
...
def masked_accuracy(y_true, y_pred):
    ...
    # Create the mask, i.e., the values that will be ignored (padding positions where y_true == 0)
    mask = tf.not_equal(y_true, 0)
    mask = tf.cast(mask, tf.float32)

    # Perform argmax to get the predicted class index at each time-step
    ...
    # Multiply the acc tensor with the masks (zero out padding positions)
    matches_true_pred *= mask

    # Compute masked accuracy (quotient between the total matches and the total valid values,
    # i.e., the amount of non-masked values)
    masked_acc = tf.reduce_sum(matches_true_pred) / tf.maximum(tf.reduce_sum(mask), 1.0)
...
def predict(sentence, model, sentence_vectorizer, tag_map):
    ...
    # Expand its dimension to make it appropriate to pass to the model  (add batch dim)
    sentence_vectorized = tf.expand_dims(sentence_vectorized, 0)

    # Get the model output  — shape: (1, seq_len, num_classes)
...
def main(train_sentences, val_sentences, test_sentences,
    ...
    model.save('saved_model.h5')     # Required for submission (*.h5 format)
...
if __name__ == '__main__':
    ...
    def print_usage():
        ...
        print("")
        print("Example: ")
        print("$> python3 assignment6.py large/train/sentences.txt large/val/sentences.txt large/test/sentences.txt \\")
        print("                          large/train/labels.txt large/val/labels.txt large/test/labels.txt")
        return
